...
protected:
  ...
  // Создание
public:
///CPeerNetDialog(CWnd *pParent = nullptr);	// стандартный конструктор
  CPeerNetDialog(CWnd *pParent = NULL);	// стандартный конструктор

  // Добавить строку в таблицу голосования
  bool  AddInVotingTable(LPTSTR pszValues[], bool bClear = false)
  { return AddRowInListView(&m_lsvVoting, VOTECOLNUM, pszValues, bClear); }
  // Очистить таблицу транзакций
  //void  ClearTransactTable() { ClearListView(&m_lsvTransacts); }
  // Добавить строку в таблицу транзакций
  bool  AddInTransactTable(TTransact *ptRansact);
  //bool  AddInTransactTable(LPTSTR pszValues[], bool bClear = false)
  //{ return AddRowInListView(&m_lsvTransacts, TRANCOLNUM, pszValues, bClear); }
...
private:
  ...
  bool  AddRowInListView(CListViewEx *pLsv, int nCols,
