...
/* Добавить пользователя в список владельцев узлов
void  CPeerNetDialog::AddToUserList(WORD iUser)
{
  //WORD iOwner = m_oNode.GetNodeOwner(iNode);
  TUser *ptUser = m_oBlkChain.UserRegister()->GetAt(iUser);
  CString s(ptUser->_sName);
  m_cmbUserTo.AddString(s.GetBuffer()); //список пользователей-получателей
} // CPeerNetDialog::AddToUserList()

// Добавить новый узел в список узлов сети
void  CPeerNetDialog::AddToNodeList(WORD iNode)
{
  TCHAR szMessBuf[ERMES_BUFSIZE];
  // Сформировать в буфере обозначение узла
  swprintf_s(szMessBuf, ERMES_BUFSIZE, _T("Node #%d"), iNode);
  m_lstNodes.AddString(szMessBuf);
  m_lstNodes.UpdateWindow();
} // CPeerNetDialog::AddToNodeList()

// Удалить узел из списка узлов сети
void  CPeerNetDialog::DeleteFromNodeList(WORD iNode)
{
  TCHAR szMessBuf[ERMES_BUFSIZE];
  // Сформировать в буфере обозначение узла
  swprintf_s(szMessBuf, ERMES_BUFSIZE, _T("Node #%d"), iNode);
  int i = m_lstNodes.FindString(-1, szMessBuf); //искать с начала списка
  if (i == LB_ERR)  return;
  m_lstNodes.DeleteString((UINT)i);
  m_lstNodes.UpdateWindow();
} // CPeerNetDialog::DeleteFromNodeList() */

// Добавить строку в таблицу транзакций
bool  CPeerNetDialog::AddInTransactTable(TTransact *ptRansact)
{
/*
...
*/
  ...
  return AddRowInListView(&m_lsvTransacts, TRANCOLNUM, pszFields);
...
void  CPeerNetDialog::ShowOwnNodeNumber(WORD iOwnNode)
{
  ...
  m_iOwnNode = iOwnNode;
  // Сформировать в буфере обозначение узла
  swprintf_s(szMessBuf, ERMES_BUFSIZE, _T("Peer Net Node #%d"), iOwnNode);
...
{
  m_sReply.Format(_T("Node #%d: %s"), iNode, lpszReply);
...
bool  CPeerNetDialog::AddRowInListView(CListViewEx *pLsv, int nCols,
...
bool  CPeerNetDialog::FillTransactTable()
{
  ClearListView(&m_lsvTransacts);
  TTransact *ptRansact;  bool b = true;
/*  WORD iUser;  CString sTransNo, sAmount;
  TCHAR szUserFrom[NAMESBUFSIZ], szUserTo[NAMESBUFSIZ],
        *pszFields[TRANCOLNUM];
  CUserRegister *prUser = m_oBlkChain.UserRegister(); */
  CTransRegister *prTrans = m_oBlkChain.TransRegister();
...
/*    sTransNo.Format(_T("%d"), ptRansact->GetTransOrd());
    ...
    sAmount.Format(_T("%.2lf"), ptRansact->GetAmount());
    pszFields[3] = sAmount.GetBuffer();     //сумма */
    b = b && AddInTransactTable(ptRansact);
  }
  return b;
...
BOOL  CPeerNetDialog::OnInitDialog()
{
  ...
  for (i = 0; i < 4; i++)
    b = AddRowInListView(&m_lsvVoting, VOTECOLNUM, pszRow);
  for (i = 0; i < 10; i++)
    b = AddRowInListView(&m_lsvTransacts, TRANCOLNUM, pszRow+2);
/*
typedef struct {
  UINT      cbSize;
  UINT      style;
  ...
  BOOL bb = GetClassInfoEx(
	theApp.m_hInstance, //HINSTANCE hinst
    theApp., //LPCTSTR lpszClass
      //LPWNDCLASSEX lpwcx
  );
*/
#endif
...
void  CPeerNetDialog::OnBnClickedStartBtn()
{
  ...
  if (b)
  {
    ...
    ClearVotingTable();  b = FillTransactTable();
...
void  CPeerNetDialog::OnBnClickedSendBtn()
{
  ...
  else
  {
    ...
    WORD  wLeng = (WORD)sNode.GetLength(),
          wTo = CPeerNetNode::ToNumber(sNode.GetBuffer() + 6, wLeng - 6);
    ...
    if (wLeng > 0)
    {
      ...
      sReqst.Format(_T("DTB %02d%02d%04d%s"),
                    m_oNode.m_iOwnNode, wTo, wLeng+1, m_sRequest.GetBuffer());
...
void  CPeerNetDialog::OnBnClickedDumpBtn()
{
  ...
  swprintf_s(szBuffer, 128, _T("..\\TraceDump_%d.txt"), m_iOwnNode);
