...
#endif
  TMessageBuf *pmb = new TMessageBuf(m_mbReply.Message());
  return pmb;
...
{
  memset(m_mbDTB, 0, sizeof(m_mbDTB));  //обнулить массив очереди
...
bool  CCyclicQueue::Put(TMessageBuf &mbDTB)
{
  if (m_nCount == CLCQUEUESIZE)  return false;
  m_mbDTB[m_iTail].PutMessage(mbDTB.Message());
...
//  Инициализация статических элементов данных
//----------------------------------------------------------------------
//static  //NP в состоянии Listen
//CNamedPipe * CPeerNetNode::m_pNPListen = NULL;
//static  //флаг завершения потока генератора экземпляров
//bool   CPeerNetNode::m_bTerm = false;
//static  //буфер сообщения об ошибке
//TCHAR  CPeerNetNode::m_szMessBuf[] = { 0 }; //ERMES_BUFSIZE
...
//m_pNPListen = NULL;   //NP в состоянии Listen static
...
//m_nSrvNPs = 0;   //мгновенное количество подключённых серверных NPs
  memset(m_pCliNPs, 0, sizeof(m_pCliNPs));
/*  m_hMayStartClient = ::CreateEvent(
    NULL, FALSE,    //default security attribute, FALSE=auto reset event
    FALSE, NULL     //initial state = non-signaled, unnamed event object
  ); */
//m_FreeNodes.Clear();
//m_bSrvStarting = false;   //флаг режима запуска сервера
#ifdef _TRACE
  m_pTraceFile = NULL;
#endif
}
...
{
  //TCHAR szMessBuf[ERMES_BUFSIZE];
  WORD iNode; /* CNamedPipe *pNP; */ bool b, bRet = true;  UINT nRet = 0;
...
void  CPeerNetNode::ShutdownNode()
{
  ...
  if (m_iOwnNode != UNKNOWN_NODE && m_pNPListen)
  {
    ...
    // Установить соединение со "своим" сервером, чтобы вывести поток сервера 
    //из ожидания
...
/* Дать владельца узла
WORD  CPeerNetNode::GetNodeOwner(WORD iNode)
{
  ASSERT(0 <= iNode && iNode < m_nNodes);
  return (iNode == m_iOwnNode) ? m_iOwner : m_pSrvNPs[iNode]->m_iCncOwner;
} // CPeerNetNode::GetNodeOwner() */
...
void  CPeerNetNode::ConnectRemoteClient(WORD iNode, WORD iOwner, CNamedPipe *pNP)
...
//pNP    - серверный канал нашего узла kNode, которым он связан с узлом iNode
{
  ...
  StartNPClient(iNode);
  //m_pBlkChain->TieUpUserAndNode(iOwner, iNode);
  //m_pMainWin->AddToUserList(iOwner);
...
//mbRequest[in/out] - буфер сообщения (запроса)
{
  ...
  // Сформировать голову команды DTB
  ...
  WORD wLeng = (WORD)_tcslen(szReqst);
  memmove(mbRequest.Message() + wLeng,
          mbRequest.Message(), mbRequest.MessageBytes());
  memcpy(mbRequest.Message(), szReqst, wLeng * sizeof(TCHAR));
  mbRequest._wMessBytes += wLeng * sizeof(TCHAR);
...
//mbReply[in/out] - буфер сообщения (ответа на запрос)
{
  ...
  WORD wLeng = ToNumber(pmbReply->Message() + 8, 4),    //символов вместе с '\0'
       nSize = wLeng * sizeof(TCHAR);       //байт вместе с '\0'
...
/*  if (eCmd == CmC_NONE)
  {
    LPTSTR lpszFrmt = (eCmd == CmC_TRM) ? _T("TRM %02d%02d")
                                        : _T("ACK %02d%02d");
  } */
