...
#define  CLCQUEUESIZE   32      //размер циклической очереди
#define  CLCQUEUEMASK   0x001F  //маска выделения циклического индекса
...
//  CmC_CNC,             Confirm connection - Подтвердить соединение
  CmC_DTB,          //2: Data block         - Блок данных
  CmC_SCL,          //3: Start client       - Запустить клиент
  CmC_TRM,          //4: Terminate          - Закрыть канал
};

/* Идентификаторы (индексы в массиве) событий синхронизации
enum EEventId {
  EvTermnReqst = 0, //события "Запрос на закрытие канала"
  EvReqstReady,     //события "Запрос к серверу готов"
  EvRepReceivd,     //события "Ответ сервера получен"
}; */
...
public:
  ...
  WORD          m_iCncNode;
  // Владелец корреспондирующего узла - определён только в серверных каналах
  //WORD          m_iCncOwner;
...
{
  // Данные
  TMessageBuf  m_mbDTB[CLCQUEUESIZE];   //очередь принятых блоков DTB
...
public:
  ...
  bool  Put(TMessageBuf &mbDTB);
  bool  Get(TMessageBuf *&pmb);
private:
...
class CPeerNetNode
{
  /* Структурный тип "Специализированный односвязный список"
  struct TList {
    WORD  _iElems[NODECOUNT];     //односвязный список номеров (0..N-1)
    WORD  _iFirst;              //индекс первого элемента списка
    WORD  _iLast;               //индекс последнего элемента списка
    WORD  _nCount;              //количество элементов списка
    // Очистить список
    void  Clear() {
      memset(_iElems, 0xFE, sizeof(_iElems));
      _iLast = _iFirst = 0xFFFF;  _nCount = 0;
    }
    // Вставить новый элемент в конец списка
    bool  PutInTail(WORD i) {
      bool b = (0 <= i && i < NODECOUNT && _nCount < NODECOUNT);
      if (b) {
        if (_nCount == 0)  _iFirst = i;
        else  _iElems[_iLast] = i;
        _iElems[i] = 0xFFFF;  _iLast = i;  _nCount++;
      }
      return b;
    }
    // Вставить новый элемент в начало списка
    bool  PutInHead(WORD i) {
      bool b = (0 <= i && i < NODECOUNT && _nCount < NODECOUNT);
      if (b) {
        if (_nCount == 0)  _iLast = i;
        _iElems[i] = _iFirst;  _iFirst = i;  _nCount++;
      }
      return b;
    }
    // Извлечь из списка первый элемент
    bool  GetFromHead(WORD &i) {
      bool b = (0 < _nCount);
      if (b) {
        i = _iFirst;  _iFirst = _iElems[i];  _iElems[i] = 0xFEFE;
        if (--_nCount < 2)  _iLast = _iFirst;
      }
      return b;
    }
  };
  // Структурный тип "Описатель серверного именованного канала"
  struct TServerNP {
    WORD        _iNode;     //индекс узла
    CNamedPipe *_pServNP;   //серверный NP узла
  }; */
public:
//static TCHAR  m_szMessBuf[ERMES_BUFSIZE]; //общий буфер сообщения об ошибке
