...
public:
  ...
  // Дать пользователя по его хеш-коду
...
struct TTransact {
  ...
  void  Clear() {
    memset(_chPrevHash, 0, sizeof(TTransact));
  }
  ...
  void  SetTransOrd(WORD nOrd) {
    ...
    int  len = min(strlen(_szTrnsOrd), NAMESBUFSIZ-1);
    // Обнуление хвоста (как минимум один ноль)
    memset(_szTrnsOrd+len, 0, NAMESBUFSIZ-len);
  }
  WORD  GetTransOrd() {
    DWORD nOrd;
  ...
  void  SetAmount(double rSum) {
    sprintf_s(_szAmount, NAMESBUFSIZ, "%.2f", rSum);
    int  len = min(strlen(_szAmount), NAMESBUFSIZ-1);
    // Обнуление хвоста (как минимум один ноль)
    memset(_szAmount+len, 0, NAMESBUFSIZ-len);
  }
  double  GetAmount() {
    double rSum;
...
public:
  ...
  // Провести транзакцию
  //void  ExecuteTranscation(WORD iUserFrom, WORD iUserTo, double rAmount);
protected:
  // Дать образец сигнатуры файла БД
  virtual char * GetSignaturePattern(WORD &nLeng);
private:
